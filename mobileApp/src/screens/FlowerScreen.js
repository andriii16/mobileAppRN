...
import {FlowerService, StaticImageService} from "../services";
...
const FlowerScreen = ({
  ...
  useEffect(() => {
    FlowerService.getOneFlowerById(flowerId).then(response => {
      console.log(response?.data);
      setFlower(response?.data);
    });
  }, []);

  const addToCart = flowerId => dispatch(CartAction.addToCart({flowerId}));
  const removeFromCart = flowerId =>
    dispatch(CartAction.removeFromCart({flowerId}));
      ...
      <ScrollView>
        ...
        <View style={styles.mainContainer}>
          <View style={styles.titleHeaderContainer}>
            <Text style={styles.titleText}>{flower?.name}</Text>
          ...
          <View style={styles.subHeaderContainer}>
            ...
            <View style={styles.rowAndCenter}>
              ...
              <Text style={styles.deliveryText}>Besplatna isporuka</Text>
            </View>
          </View>
          ...
          <View style={styles.detailsContainer}>
            {flower?.description ? (
              <>
                <Text style={styles.detailHeader}>Opis</Text>
                <Text style={styles.detailContent}>{flower?.description}</Text>
              </>
            ) : null}
          </View>
        </View>
      ...
      <View style={styles.buttonsContainer}>
        <View style={styles.itemAddContainer}>
          <AntDesign
            ...
            color={Colors.RED}
            size={18}
            onPress={() => removeFromCart(flowerId)}
          />
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}>
          <Text style={styles.cartButtonText}>Otvorite korpu</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
