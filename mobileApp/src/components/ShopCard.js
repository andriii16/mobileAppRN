...
const ShopCard = ({
  ...
  const isBookmarked = useSelector(
    state =>
      state?.bookmarkState?.bookmarks?.filter(item => item?.shopId === id)
        ?.length > 0,
  );
  const addBookmark = () => dispatch(BookmarkAction.addBookmark({shopId: id}));
  const removeBookmark = () =>
    dispatch(BookmarkAction.removeBookmark({shopId: id}));
  return (
    <TouchableOpacity
      ...
      <Text style={styles.tagText}>{tags?.slice(0, 4).join(" • ")}</Text>
